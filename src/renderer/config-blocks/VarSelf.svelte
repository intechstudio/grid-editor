...
<script>
  ...
  */
  ...
  function selfArrayToScript(arr){
    let script = [(arr.map(e => "self."+e.variable)).join(','), '=', arr.map(e => e.value).join(',')].join('');
    return script;
  }
  ...
  function selfsToConfig({script}){  
    ...
    if(parenthesis(script)){
      ...
      _variable_array = _variable_array.map(e => {return e.split('.')[1]})
...
<config-self-definitions   
    ...
    {#each scriptSegments as script, i (i)}
      <div class="w-full h-full flex self-defs py-2">
        ...
        <div class="w-1/12 pl-1 flex items-center justify-center">
          {#if i !== 0}
            <button on:click={()=>{removeSelfVariable(i)}} class="flex items-center group cursor-pointer pl-1">
  ...
  <div class="w-full flex group p-2">
    <button on:click={()=>{addSelfVariable()}} class="group-hover:border-pick cursor-pointer group-hover:bg-select-saturate-10 border-secondary transition-colors duration-300 w-full border-l-4 text-white pl-4 py-0.5">
      Add self variable...
    </button>
  </div>
