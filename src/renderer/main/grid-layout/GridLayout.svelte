<script>
  ...
  function calculate_x(x0, y0) {
    ...
    return x_rot * 106.6 * $appSettings.size;
  }
  function calculate_y(x0, y0) {
    ...
    return -1 * (y_rot * 106.6 * $appSettings.size);
  }
...
<layout-container>
  <div
    class="flex items-start {$$props.class} {$engine == 'ENABLED'
      ? 'pointer-events-auto'
      : 'pointer-events-none'}"
  >
    <grid-layout
      class="overflow-hidden flex flex-col h-full w-full
    focus:outline-none border-none outline-none bg-red-500"
    >
      <Pages class="w-full" />
      <section class="inline-block mx-auto my-8">
        <div
