...
<layout-container
  class="relative flex items-start {classes} h-full {$engine == 'ENABLED' ? 'pointer-events-auto' : 'pointer-events-none'}">
  <grid-layout
    ...
    <div
      ...
      style="top:{map_h / 2 - gridsize / 2}px; left:{map_w / 2 - gridsize / 2}px;"
      ...
      <div
        ...
        {#each $devices as device}
          <div
            in:fly={{ x: calculate_x(device.fly_x, device.fly_y), y: calculate_y(device.fly_x, device.fly_y), duration: 150 }}
