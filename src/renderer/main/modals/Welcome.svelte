...
<modal class=" z-40 flex absolute items-center justify-center w-full h-screen bg-secondary bg-opacity-50">
  ...
  <div bind:this={modalElement}  use:clickOutside={{useCapture:true}} on:click-outside={()=>{$appSettings.modal = ''}}  id="clickbox" 
    class="items-center z-50 w-3/5 text-white relative flex flex-col shadow bg-primary bg-opacity-100 opacity-100 max-w-4xl">
      ...
      <div bind:this={attachmentElement} class="flex flex-row bg-primary w-full" style="">
        <div  class="px-2 flex flex-row w-full bg-black bg-opacity-20">
          ...
          <div class="p-4 flex-col w-5/12 flex justify-between">
            ...
            <div 
              on:click={e => window.electron.openInBrowser(env.DOCUMENTATION_REFERENCEMANUAL_URL)} 
              class="flex w-full text-blue-500 cursor-pointer">
              Editor reference manual...
            </div>
            <div 
              on:click={e => window.electron.openInBrowser(env.DOCUMENTATION_DISCORDSERVER_URL)} 
              class="flex w-full text-blue-500 cursor-pointer">
            ...
            <div 
              on:click={e => window.electron.openInBrowser(env.DOCUMENTATION_TROUBLESHOOTING_URL)} 
              class="flex w-full text-blue-500 cursor-pointer">
              Grid does not connect...
            </div>
            <div 
              on:click={e => window.electron.openInBrowser(env.DOCUMENTATION_FIRMWAREUPDATE_URL)} 
              class="flex w-full text-blue-500 cursor-pointer">
              Updating the firmware...
            </div>
            <div 
              on:click={e => window.electron.openInBrowser(env.DOCUMENTATION_MAINTENANCE_URL)} 
              class="flex w-full text-blue-500 cursor-pointer">
           ...
           <div 
            class="flex w-full">
            <div on:click={e => window.electron.openInBrowser(env.PUBLIC_ROADMAP_URL)}  class="rounded py-2 px-4 my-2  bg-secondary text-white cursor-pointer">Public Roadmap</div>
          </div>
      ...
      <div class="flex flex-row w-full bottom-0 bg-black bg-opacity-10 justify-between items-center"> 
       
        <div class="flex flex-col h-full p-6">
            <div class="flex w-full opacity-70 ">Grid Editor is Open-Source Software</div>
            <div on:click={e =>window.electron.openInBrowser(env.EDITOR_REPOSITORY_URL)} class="flex w-full opacity-40 hover:opacity-100 transition-opacity cursor-pointer">
              <svg class="mr-2 w-5 h-5" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex flex-row items-center h-full p-6">
          <input class="mr-1 opacity-70 " type="checkbox" bind:checked={$appSettings.persistant.welcomeOnStartup}>
          <div class="mx-1 mr-4 opacity-70 ">Always show on startup</div>

          <button on:click={()=>{$appSettings.modal = ''}} id="close-btn" 
            class="px-3 py-1  cursor-pointer rounded not-draggable hover:bg-blue-700 bg-blue-500">
            Close
          </button>
        </div>
