...
<config-debug class="w-full h-full flex flex-col p-4 z-10 bg-primary">
  ...
  <textarea spellcheck="false" bind:value={websocketMessage} class="w-full cursor-default min-h-36 h-36 bg-secondary rounded px-1 my-2 text-white font-mono"/>

  <button 
    class="px-4 py-1 my-4 bg-select hover:bg-select-saturate-10 rounded" 
    on:click={sendMessage}>Send
  </button>
