...
<profiles
  ...
  <div
    ...
    <button
      ...
      class="{!checkIfOk(newProfile) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer opacity-100  hover:bg-commit-saturate-10'}
      ...
      <ul class="w-full">
        ...
        {#if PROFILES.length === 0}
          <div in:fade={{ delay: 500 }} class="text-yellow-500">
            <b>Profiles not found!</b>
          </div>
          <div in:fade={{ delay: 1000 }} class="text-yellow-500">
            Setup profile folder and download the default profile library in the
            Preferences menu...
          </div>
        {/if}
        ...
        {#each PROFILES.sort(compare) as profile, i}
          <li
            ...
            class="{selectedIndex == i ? 'border-pick bg-secondary' : 'bg-secondary bg-opacity-40 border-primary hover:bg-opacity-70 hover:border-pick-desaturate-10'}
            border-l-4 profile p-2 my-2 cursor-pointer relative">
            ...
            <div class="flex text-xs opacity-80 font-semibold">
              <div
                ...
                inline-block {newProfile.type == profile.type ? 'bg-green-500' : 'bg-gray-600'}
                ">
                {profile.type}
              </div>
              <div
                ...
                v{profile.version.major}.{profile.version.minor}.{profile.version.patch}
              </div>
              <div
                class="m-1 flex justify-center text-center rounded-full px-2
                inline-block"
  ...
  <button
    ...
    class="relative bg-commit block {selectedIndex !== undefined ? 'hover:bg-commit-saturate-20' : 'opacity-50 cursor-not-allowed'}
