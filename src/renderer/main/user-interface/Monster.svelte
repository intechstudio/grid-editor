<script>
	...
	const colors = [`#68bf7a`, `#9889c0`, `#f9be21`, `#6e9ed4`];
			...
			if ($attachment.element.getBoundingClientRect !== undefined) {
				...
				if (typeof vpos !== 'undefined' && vpos !== null) {
					if (vpos.endsWith('%') && !isNaN(parseInt(vpos))) {
						_helperY =
							bounding.top +
							(bounding.height / 100) * parseInt(vpos) -
							monsterElement.parentElement.getBoundingClientRect().top;
					}
				}
				...
				if (typeof hpos !== 'undefined' && hpos !== null) {
					if (hpos.endsWith('%') && !isNaN(parseInt(hpos))) {
						_helperX =
							bounding.left +
							(bounding.width / 100) * parseInt(hpos) -
							monsterElement.parentElement.getBoundingClientRect().left;
					}
				}

				if (
					typeof $attachment.scale !== 'undefined' &&
					$attachment.scale !== null
				) {
	...
	function refreshEyes() {
		...
		let X =
			...
			helperX -
			50 +
			(eyeOrigin[_shapeSelected].x1 + eyeOrigin[_shapeSelected].x2) / 2 -
			monsterElement.parentElement.getBoundingClientRect().left;
		let Y =
			lastMouseY -
			helperY -
			50 +
			(eyeOrigin[_shapeSelected].y1 + eyeOrigin[_shapeSelected].y2) / 2 -
			monsterElement.parentElement.getBoundingClientRect().top;
...
<div
	...
	class="absolute bg-gray-500 bg-opacity-0"
	style="z-index: 70; top: {helperY - (100 / 2) * scale}px; left: {helperX -
		(100 / 2) * scale}px;"
>
			...
			<g id="b">
				<g id="c">
					...
					<g class={helperSleep ? 'hidden' : ''}>
						...
						<circle
							class="d"
							cx={eyeOrigin[_shapeSelected].x1}
							cy={eyeOrigin[_shapeSelected].y1}
							r="9.5"
						/>
						<circle
							cx={eyeOrigin[_shapeSelected].x1 + eyesDX}
							cy={eyeOrigin[_shapeSelected].y1 + eyesDY}
							r="5"
						/>
						...
						<circle
							class="d"
							cx={eyeOrigin[_shapeSelected].x2}
							cy={eyeOrigin[_shapeSelected].y2}
							r="9.5"
						/>
						<circle
							cx={eyeOrigin[_shapeSelected].x2 + eyesDX}
							cy={eyeOrigin[_shapeSelected].y2 + eyesDY}
							r="5"
						/>

						<!-- Normal Mouth -->
						<path
							transform="translate({(eyeOrigin[_shapeSelected].x1 +
								eyeOrigin[_shapeSelected].x2) /
								2 -
								50},{(eyeOrigin[_shapeSelected].y1 +
								eyeOrigin[_shapeSelected].y2) /
								2 -
								50})"
							class="f"
							d="M55,65c0,2.76-2.24,5-5,5s-5-2.24-5-5"
						/>
					</g>
					...
					<g class={helperSleep ? '' : 'hidden'}>
						...
						<path
							transform="translate({(eyeOrigin[_shapeSelected].x1 +
								...
								50})"
							class="f"
							d="M43.5,45.75c0,4.69-3.81,8.5-8.5,8.5s-8.5-3.81-8.5-8.5"
						/>
						<path
							transform="translate({(eyeOrigin[_shapeSelected].x1 +
								eyeOrigin[_shapeSelected].x2) /
								...
								50})"
							class="f"
							d="M73.5,45.75c0,4.69-3.81,8.5-8.5,8.5s-8.5-3.81-8.5-8.5"
						/>
