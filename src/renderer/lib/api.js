...
async function send({ method, path, data }) {
    ...
    async function retryRequest(url, opts) {
        // get a new token
        const response = await api.post('auth/exchange-token', { refreshToken: get(appSettings).persistant.authRefreshToken })
        if (response.ok == true) {
    ...
    function wrappedFetch(url, opts) {
        return fetch(url, opts)
            ...
            .catch(err => console.error(`Fetch Failed, API error from ${apiUrl}/${path}: ${err}`));
    }
