...
<wrapper class="flex flex-row my-2">
  {#if type == 'standard'}
  <carousel on:click={()=>{toggle = ! toggle}} class="flex flex-grow text-white cursor-pointer ">
    <name style="background-color: {color}" class="flex items-center py-2">
      <span class="px-2 ">icn</span>
    </name>
    {#if !toggle}
  ...
  {/if}
  {:else if type == 'dummy'}
    <div class="w-full py-2 invisible">
      dummy
    </div>
  {:else}
