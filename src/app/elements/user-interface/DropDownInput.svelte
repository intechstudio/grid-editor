...
<main class="w-full relative" use:clickOutside on:click-outside={()=>{focus = false}}>
  <input class:shadow={focus} on:click={()=>{focus = true}} on:change={handleChange} bind:value={dropDownValue} on:input={(e)=>{focus = false; handleChange()}} type="text" class="w-full bg-secondary text-white p-1 pl-2 rounded-none focus:outline-none">
  {#if focus}
