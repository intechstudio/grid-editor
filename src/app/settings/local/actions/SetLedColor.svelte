<script>
  ...
  onMount(()=>{
    ...
    if(action.parameters[2] === ''){
      action.parameters[1] = ['','']
      action.parameters[2] = 255;
      action.parameters[3] = 0;
      action.parameters[4] = 0;
      action.parameters[5] = 1;
    } 

    startColor = `rgb(${action.parameters[2]}, ${action.parameters[3]}, ${action.parameters[4]})`

    alpha = action.parameters[5];

    action.parameters[0] = parameters[0].value;
  ...
  function sendData(){
    ...
    for (let i = 0; i < action.parameters[1].length; i++) {
      ...
      const parameters = [
        ...
        { 'RED': Math.floor(action.parameters[2] * action.parameters[5]) },
        { 'GRE': Math.floor(action.parameters[3] * action.parameters[5]) },
        { 'BLU': Math.floor(action.parameters[4] * action.parameters[5]) }
      ];
  ...
  function colorCallback(rgba) {
    ...
    action.parameters[2] = Math.floor(rgba.detail.r)
    action.parameters[3] = Math.floor(rgba.detail.g)
    action.parameters[4] = Math.floor(rgba.detail.b)
    action.parameters[5] = rgba.detail.a;
  ...
  <div class="flex w-full text-white">
    <div class="flex w-5/12 flex-col xl:flex-row">
      <div class="w-full xl:w-5/12">
        ...
        <DropDownInput optionList={parameters} bind:dropDownValue={action.parameters[0]}/>
      </div>
    ...
    <div class="flex w-7/12 flex-col xl:flex-row">
      <div class="px-1">
        ...
        <input type="number" bind:value={action.parameters[2]} on:change={sendData} min=0 max=255 class="w-full secondary text-white p-1 pl-2 rounded-none focus:outline-none">
      </div>
      <div class="px-1">
        <div class="text-gray-700 text-xs">Green</div>
        <input type="number" bind:value={action.parameters[3]} on:change={sendData} min=0 max=255 class="w-full secondary text-white p-1 pl-2 rounded-none focus:outline-none">
      </div>
      <div class="px-1">
        <div class="text-gray-700 text-xs">Blue</div>
        <input type="number" bind:value={action.parameters[4]} on:change={sendData} min=0 max=255 class="w-full secondary text-white p-1 pl-2 rounded-none focus:outline-none">
      </div>
      <div class="px-1 xl:pr-2">
        <div class="text-gray-700 text-xs">Alpha</div>
        <input type="number" bind:value={action.parameters[5]} on:change={sendData} min=0 max=1 class="w-full secondary text-white p-1 pl-2 rounded-none focus:outline-none">
      </div>
    </div>
  </div>
