...
<nav-tab style="background-color:rgb(25, 26, 32)" class=" flex px-1 flex-col items-center justify-between  h-full">
  <div class="flex flex-col">
    ...
    <div 
      ...
      class="relative cursor-pointer mx-1 mb-2 p-1 w-14 h-14 flex justify-center items-center group rounded-lg transition hover:bg-opacity-100 {selectedRightTab == 'Configuration' ? 'bg-opacity-100' : 'bg-opacity-40'} bg-secondary ">
      <svg 
      ...
      <div class="left-0 -ml-3 absolute transition-all  {selectedRightTab == 'Configuration' ? 'h-8' : 'h-2 group-hover:h-4'} w-2 rounded-full bg-white"></div>
      <TooltipSetter key={"sidebar_configuration_icon"}/>
    </div>

    <div 
      on:click={()=>{changeRightTab('Preferences')}}  
      class="relative cursor-pointer m-1 my-2 p-1 w-14 h-14 flex justify-center items-center group transition hover:bg-opacity-100 rounded-lg {selectedRightTab == 'Preferences' ? 'bg-opacity-100 ' : 'bg-opacity-40 '} bg-secondary">
      <svg 
      ...
      <div class="left-0 -ml-3 absolute transition-all  {selectedRightTab == 'Preferences' ? 'h-8' : 'h-2 group-hover:h-4'} w-2 rounded-full bg-white"></div>
  ...
  <div class="flex flex-col">
    ...
    <div 
      ...
      class="relative cursor-pointer m-1 my-2 p-1 w-14 h-14 flex justify-center items-center group rounded-lg transition hover:bg-opacity-100 {selectedLeftTab == 'Debug' ? 'bg-opacity-100' : 'bg-opacity-40'} bg-secondary ">
      ...
      <div class="left-0 -ml-3 absolute transition-all  {selectedLeftTab == 'Debug' ? 'h-8' : 'h-2 group-hover:h-4'} w-2 rounded-full bg-white"></div>
      <TooltipSetter key={"sidebar_debugger_icon"}/>

    </div>

    <div 
    on:click={()=>{changeLeftTab('MIDI Monitor')}}  
    class="relative cursor-pointer m-1 my-2 p-1 w-14 h-14 flex justify-center items-center group transition hover:bg-opacity-100 rounded-lg {selectedLeftTab == 'MIDI Monitor' ? 'bg-opacity-100 ' : 'bg-opacity-40 '} bg-secondary">
      <svg 
    ...
    <div class="left-0 -ml-3 absolute transition-all  {selectedLeftTab == 'MIDI Monitor' ? 'h-8' : 'h-2 group-hover:h-4'} w-2 rounded-full bg-white"></div>
