    ...
    {#each  [0,1,2,3] as row }
      <div class="control-row" style="--control-row-mt: {$appSettings.size * 3.235 +'px'}; --control-row-mx: {$appSettings.size * 6.835 + 'px'}; --control-row-mb: {$appSettings.size * 6.835 + 'px'}" >
        {#each  [0,1,2,3] as elementNumber}
          <div class:active-element={dx == selectedElement.brc.dx && dy == selectedElement.brc.dy && selectedElement.event.elementnumber == row * 4 + elementNumber} class="knob-and-led">
            <Led    
              color={ledcolor_array[elementNumber+4*row]} 
              size={$appSettings.size}/>
            <Potentiometer
              position={elementposition_array[elementNumber+4*row]} 
              size={$appSettings.size}/>
          </div>
        {/each}
      </div>
