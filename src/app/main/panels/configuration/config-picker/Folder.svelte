...
<div class="flex {name == "Presets" ? '' : 'pl-4'} items-start flex-col w-full">

	{#if name == "Presets"}
		<div class="flex items-center py-1 w-full text-gray-700 text-sm mb-1">
	...
	{#if expanded}
		...
		<ul class="text-white w-full">
			...
			{#each configs as [key, value]}
				...
				<li class="{`bg-opacity-${counter*25}`} bg-secondary py-1 px-2 my-1 flex flex-col rounded-lg  w-full">

						{#if isDeeper(key, value)}
							<svelte:self name={key} {counter} {index} configs={Object.entries(value)}/>
						{:else}
							<Preset {index} {counter} {...value}/>
						{/if}
