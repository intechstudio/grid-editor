...
{#if (componentName == 'Locals' || componentName == 'CodeBlock') && !$appMultiSelect.enabled}
  <show-advanced id="show-advanced" on:click={(e)=>{e.preventDefault(); actionPrefStore.showAdvanced(index)}} class="flex pl-2 group justify-center  items-center bg-transparent">
