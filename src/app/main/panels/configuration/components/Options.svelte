...
{#if (componentName == 'Locals' || componentName == 'CodeBlock') && !$appMultiSelect.enabled}
  <show-advanced id="show-advanced" on:click={(e)=>{e.preventDefault(); actionPrefStore.showAdvanced(index); componentName == 'CodeBlock' ? toggle = false : null; console.log('TOGGLE', toggle, componentName)}} class="flex pl-2 group justify-center  items-center bg-transparent">
