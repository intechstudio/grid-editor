...
<app-action-multi-select class=" flex items-center justify-end flex-wrap">
  ...
  <BtnAndPopUp on:clicked={()=>{configManagement().on_click.copy(); appMultiSelect.reset();}} btnStyle={`relative bg-pick hover:bg-pick-saturate-10 mr-2 rounded-full ${$appMultiSelect.selection.includes(true) ? '' : 'opacity-50 pointer-events-none'}`} popStyle={'bg-pick '}> 
  ...
  <BtnAndPopUp on:clicked={()=>{configManagement().on_click.paste(); appMultiSelect.reset();}} btnStyle={`relative bg-purple-500 hover:bg-purple-600 mr-2 rounded-full ${$appActionClipboard.length > 0 ? '' : 'opacity-50 pointer-events-none'}`} popStyle={'bg-purple-500 '}> 
  ...
  <BtnAndPopUp on:clicked={()=>{configManagement().on_click.remove(); appMultiSelect.reset();}} btnStyle={`relative bg-red-500 hover:bg-red-600 mr-2 rounded-full ${$appMultiSelect.selection.includes(true) ? '' : 'opacity-50 pointer-events-none'}`} popStyle={'bg-red-500 '}> 
  ...
  <div 
    ...
    class="{$appMultiSelect.all_selected  ? 'bg-pick' : ''} relative flex items-center justify-center p-2 w-6 h-6 border-2 border-pick rounded-full text-white cursor-pointer text-xs">
