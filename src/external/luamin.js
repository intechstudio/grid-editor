...
function CreateLuaParser(text) {
  ...
  function functionargs(locals, upvals) {
    ...
    if (tk.Source == "(") {
      ...
      let argList = [];
      let argCommaList = [];
      let argExpected = false;
      while (peek().Source != ")" || argExpected) {
    ...
    if (expr.Type == "BinopExpr") {
      ...
      if (token != null) {
        if (token.Type == "Number") {
          ...
          if (int !== null && isFinite(int)) token.Source = int.toString();
        }
...
// Yes, thanks Herrt#3868 for your epic work, but watermark eats up many characters in configs! Get in touch if you would like a module @intechstudio!
